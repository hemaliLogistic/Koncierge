...
import { setFilteredUserList1, setNewMessage } from "@/redux/Chat/ChatSlice";
...
const SettingsChat = () => {
            ...
            dispatch(
                setFilteredUserList1(
                    filteredUserList.map((user) =>
                        user.id === a?.data?.userId ? { ...user, isUserOnline: true } : user
                    )
                )
            );
    ...
    return (
        <div className="settings-main-container">
            ...
            {selectedUser?.id ? <MessageList /> : <> Please Select Chat From the list to start conversation</>}
        </div>
    );
};
