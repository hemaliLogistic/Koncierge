...
const DataTableComponent = ({ data, isLoading, isPayment, page }) => {
    ...
    const column = [
        ...
        },
        {
            ...
            selector: (row, index) => row.bookDate,
            cell: (row, index) => (
                <div className='table-main-div'>
                    <span index={index} className='table-text'>
                        {isPayment ? moment(row?.transferDate).format("DD/MM/YYYY") : moment(row?.bookDate).format("DD/MM/YYYY")}
                    </span>
                </div>
            ),
            sortable: true,
        ...
        },
        {
            ...
            cell: (row, index) => (
                <div className='table-main-div'>
                    <span index={index} className='table-text'>
                        {isPayment ? moment(row?.transferTime, "HH:mm:ss").format("hh:mm A") : moment(row?.requestTime, "HH:mm:ss").format("hh:mm A")}
                        { }
                    </span>
                </div>
            ),
                    ...
                    {row?.bookingStatus === "View Quote" ? (
                        <button
                            ...
                            <span
                                className={
                                    !isPayment
                                        ? `table-main-div table-button-text w-35 h-6 ${row?.bookingStatus === "View Quote"
                                            ? "bg-green-600"
